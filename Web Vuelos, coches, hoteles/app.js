...
const travelerState = { adults: 1, children: 0, pets: 0, rooms: 1, drivers: 1 };
...
function updateTravelerText() {
  ...
  if (product === "hotel") {
    ...
    subtitle = `${people} persona${people === 1 ? "" : "s"}, ${travelerState.rooms} habitación${travelerState.rooms === 1 ? "" : "es"}`;
  }
  if (product === "package") {
    title = `${adultText}, ${childText}`;
  ...
  if (product === "car") {
    ...
    subtitle = carState.differentOffice ? "Otra oficina" : "Misma oficina";
  }
...
function renderTravelerPanel() {
  ...
  const rows =
    product === "car"
      ...
      : product === "hotel"
        ? [
          { key: "adults", title: "Adultos", sub: "18 años o más", min: 1 },
...
function renderProductExtras() {
  ...
  els.productExtraOptions.innerHTML = `
    ...
    ${carState.differentOffice ? `<label class="extra-time">
      Hora devolución

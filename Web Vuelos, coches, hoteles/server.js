...
const carLocations = [
  { name: "Madrid Barajas T4", code: "MAD-T4", country: "Espana", factor: 1 },
  { name: "Madrid Centro", code: "MAD-CENTRE", country: "Espana", factor: 1.06 },
  { name: "Barcelona El Prat", code: "BCN-APT", country: "Espana", factor: 0.98 },
  { name: "Paris Charles de Gaulle", code: "CDG-CAR", country: "Francia", factor: 1.16 },
];

const hotelLocations = [
  { name: "Paris", code: "PAR-CITY", country: "Francia", factor: 1.08 },
  { name: "Centro Paris", code: "PAR-CENTRE", country: "Francia", factor: 1.16 },
  { name: "Pullman Paris Tour Eiffel", code: "PAR-PULLMAN", country: "Francia", factor: 1.22 },
  { name: "Madrid", code: "MAD-CITY", country: "Espana", factor: 1 },
  { name: "Barcelona", code: "BCN-CITY", country: "Espana", factor: 0.96 },
];

const providerSets = {
  flight: ["Amadeus", "Kiwi.com", "Travelpayouts", "Duffel"],
  package: ["Expedia Packages", "Booking.com", "Hotelbeds", "Travelpayouts"],
  hotel: ["Hotelbeds", "Expedia", "Booking.com", "Google Hotels"],
  car: ["CarTrawler", "DiscoverCars", "Rentalcars", "Auto Europe"],
  cruise: ["CruiseDirect", "MSC Cruises", "Costa Cruceros", "Royal Caribbean"],
  train: ["Renfe", "Trainline", "SNCF Connect", "Eurostar"],
};
...
function isCommercialAirport(airport) {
  const forbidden = /\b(air base|afb|raaf|raf |naval air station|military|army|force base|aerodrome|heliport|helipad|seaplane|floatplane|closed|\[duplicate\])\b/i;
  return (
    airport.iata &&
    ["large_airport", "medium_airport"].includes(airport.type) &&
    !forbidden.test(`${airport.name} ${airport.searchText || ""}`)
  );
}
...
function searchLocations(query, limit = 12) {
  const q = normalizeText(query);
  const usableAirports = airportIndex().filter(isCommercialAirport);
  if (q.length < 2) {
...
function getCity(value) {
  ...
  const match = [...cities, ...carLocations, ...hotelLocations].find((city) => normalizeText(city.name) === normalized || normalizeText(city.code) === normalized);
...
function buildOffer(product, criteria, index) {
  ...
  const babySeats = Math.max(0, Number(guests.babySeats) || 0);
  ...
  if (product === "package") {
    stops = distance > 1.2 && index > 1 ? 1 : 0;
    basePrice = (86 * distance * pax + 72 * days * rooms) * weekend * flexibleDiscount * variance;
    duration = Math.round(130 + distance * 80 + stops * 70);
    title = `${fromCity.name} - ${toCity.name} con hotel incluido`;
    route = `${fromCity.code} -> ${toCity.code}`;
    detail = `${days} noche${days === 1 ? "" : "s"}, ${rooms} habitacion${rooms === 1 ? "" : "es"}, ${pax} viajero${pax === 1 ? "" : "s"}`;
    tag = index === 0 ? "Pack ahorro" : index === 1 ? "Hotel centrico" : index === 2 ? "Familias" : "Flexible";
    flexiblePolicyAvailable = index !== 0;
  }

  if (product === "hotel") {
    basePrice = 62 * toCity.factor * days * weekend * variance * rooms;
    duration = days;
    ...
    detail = `${days} noche${days === 1 ? "" : "s"}, ${rooms} habitacion${rooms === 1 ? "" : "es"}, ${pax} huesped${pax === 1 ? "" : "es"}`;
    tag = index === 0 ? "Ahorro" : index === 1 ? "Familias" : index === 2 ? "Mejor valorado" : "Cancelacion flexible";
    flexiblePolicyAvailable = index !== 0;
  }

  if (product === "car") {
    basePrice = 32 * toCity.factor * days * variance + babySeats * 9 * days;
    duration = days;
    title = `${["Economico", "Compacto", "SUV familiar", "Automatico premium"][index]} en ${toCity.name}`;
    route = `Recogida en ${toCity.code}`;
    detail = `${days} dia${days === 1 ? "" : "s"}, kilometraje incluido${babySeats ? `, ${babySeats} silla${babySeats === 1 ? "" : "s"} bebe` : ""}`;
    tag = index === 0 ? "Mas barato" : index === 1 ? "Popular" : index === 2 ? "Maletero amplio" : "Confort";
    flexiblePolicyAvailable = index !== 0;
  }

  if (product === "cruise") {
    basePrice = 210 * distance * Math.max(3, days) * variance * pax;
    duration = Math.max(3, days);
    title = `${fromCity.name} - ${toCity.name} en crucero`;
    route = `${fromCity.code} -> ${toCity.code}`;
    detail = `${duration} noche${duration === 1 ? "" : "s"}, ${rooms} camarote${rooms === 1 ? "" : "s"}, pension completa`;
    tag = index === 0 ? "Mejor tarifa" : index === 1 ? "Mediterraneo" : index === 2 ? "Familiar" : "Todo incluido";
    flexiblePolicyAvailable = index !== 0;
  }

  if (product === "train") {
    stops = distance > 1.1 && index > 1 ? 1 : 0;
    basePrice = 36 * distance * weekend * flexibleDiscount * pax * variance;
    duration = Math.round(80 + distance * 90 + stops * 25);
    title = `${fromCity.name} - ${toCity.name} en tren`;
    route = `${fromCity.code} -> ${toCity.code}`;
    detail = `${Math.floor(duration / 60)}h ${duration % 60}m, ${pax} pasajero${pax === 1 ? "" : "s"}`;
    tag = index === 0 ? "Mejor precio" : index === 1 ? "Alta velocidad" : index === 2 ? "Flexible" : "Confort";
    flexiblePolicyAvailable = index !== 0;
  }
...
function searchTravel(criteria) {
  ...
  if (!providerSets[product]) return { offers: [], dateOptions: [], summary: { cheapest: null, saving: null, bestDates: null } };
  ...
  const dateOptions = [-7, -2, 0, 4, 9].map((offset, index) => {
    ...
    return {
      ...
      price: Math.round(92 * distance * weight * ({ flight: 1, package: 2.6, hotel: 1.8, car: 0.8, cruise: 4.2, train: 0.55 }[product] || 1)),
